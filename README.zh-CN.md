...
这是一条需要反复执行的工作约定，适合写入 `AGENTS.md`。Agent 应当先整理出精确的修改方案，再由 DSH Web 打开专用审阅卡片，集中显示目标文件、简要理由、行号以及带颜色的增删内容，未修改部分会自动折叠。选择“应用修改”写入，选择“保持原文”则不修改文件。没有加载浏览器模块的客户端仍会显示紧凑的 Markdown diff。
