...
在希望共享记忆的工作区中打开 DSH 会话，然后输入：

> 在这个工作区中，新增的论文正文必须使用 `\gpt{}` 标记。这条规则也要适用于未来会话。

Agent 应当提出一份完整的 `AGENTS.md` 更新。DSH 会展示更新理由和完整内容；选择 **Update** 后写入，选择 **Skip** 则保持文件不变。

然后输入一条稳定的项目决策：

> 本论文把校准置信度定义为从链接预测分数到证据可靠性的映射。请让未来会话保留这项决策。

这应当产生一份 `.dsh-memory.md` 更新提案。类似“缩短第二句话”的一次性要求不应被保存。

最后，在完全相同的工作区目录中打开另一个会话，让它概括工作区规则与项目决策。
