...
- **不静默写入推断内容**：模型识别到持久意见后，必须展示完整提案并询问用户。
...
在希望共享上下文的工作区中打开一个 DSH 会话。

### 1. 保存工作区指令

输入：

> 每当你修改这个工作区中的文件时，结束前都要总结修改了什么以及验证了什么。这条规则也要适用于未来会话。

这描述的是 **Agent 应该如何工作**，因此 Agent 应当提出一份 `AGENTS.md` 更新。

DSH 会展示完整的文件提案及其理由。选择 **Update** 保存，或选择 **Skip** 保持文件不变。

### 2. 保存项目记忆

接下来，提供一项稳定的项目事实或决策：

> 本项目优先保证向后兼容，而不是采用新的 API。请让未来会话保留这项决策。

这描述的是 **Agent 应该知道什么项目背景**，而不是应该如何工作的规则，因此 Agent 应当提出一份 `.dsh-memory.md` 更新。

### 3. 在另一个会话中验证

在完全相同的工作区目录中打开另一个会话，然后询问：

> 你在这里应该遵循哪些工作区指令，又应该记住哪些项目决策？

新会话应当能够区分：

- 从 `AGENTS.md` 读取的工作规则；以及
- 从 `.dsh-memory.md` 读取的项目决策。

可以使用下面的判断方法：

| 问题 | 保存位置 |
|---|---|
| Agent 在不同任务中应该如何行动？ | `AGENTS.md` |
| Agent 应该知道哪些稳定事实、决策、术语或约束？ | `.dsh-memory.md` |
| 这项内容是否只对当前请求有用？ | 不保存 |

例如，“修改代码后运行相关测试”属于工作指令；“本项目支持 Node.js 22 和 24”属于项目记忆；“修复当前失败的测试”是一次性请求，不应保存。
...
模型负责判断意见是否持久以及应当归入哪个文件；`workspace_memory` 工具负责强制写入边界。模型推断的意见必须使用 `propose`，并且只有用户选择 **Update** 后才会写入。写入时，文件版本还必须与确认前观察到的版本一致。
