...
> 协助用户有意识地维护工作区共同指示与项目记忆，所有推断修改均需确认。
...
`dsh-workspace-memory` 协助用户有意识地维护同一工作区内各会话共享的两类上下文。

需要各会话共同遵循的工作约定可以整理到 `AGENTS.md`；稳定的项目事实和决策则分开保存在 `.dsh-memory.md`。这种区分有助于让行为指示保持聚焦，同时保留后续会话可能需要的事实性上下文。

当模型识别到可能值得长期保留的指示或记忆时，可以对现有文档提出精确修改。用户通过精简 diff 审阅修改，并决定是否应用。
...
它们都是会话当前工作目录下的普通 Markdown 文件。无需数据库、Embedding 或云端服务，内容仍然可以直接阅读、审查并纳入版本管理。
...
- **协助维护共享指示**：可复用的用户意见可以经过审阅，整合到现有 `AGENTS.md` 中。
- **区分指示与事实性上下文**：行为规则保存在 `AGENTS.md`，事实和决策则进入 `.dsh-memory.md`。
...
它与 DSH 标准 `agent-instructions` 加载器配合工作。DSH 仍负责发现并应用 `AGENTS.md`；`dsh-workspace-memory` 协助用户维护 cwd 层级的文件，并将事实性上下文分开保存在 `.dsh-memory.md`。
